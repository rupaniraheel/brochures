#!/usr/bin/env python3
"""Archive Octara PDFs exposed by paginated Google search queries."""
...
def search_result_urls() -> set[str]:
    ...
    for position, search_url in enumerate(search_pages, 1):
        ...
        # Decode common escaping used in Google's HTML and embedded JSON.
        ...
        print(
            f"Google page {position}/{len(search_pages)}; "
            f"unique PDF results: {len(found)}"
        )
    return found
...
def main() -> None:
    ...
    print(f"Discovered {len(urls)} unique PDF URLs from Google results and the existing manifest")
